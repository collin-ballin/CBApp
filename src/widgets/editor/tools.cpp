...
//  "_shape_add_vertex"
//
uint32_t Editor::_shape_add_vertex(const ImVec2& ws)
{
...
size_t Editor::_shape_build_ellipse(const ImVec2& cen, float r)
{
    ...
    uint32_t v0 = _shape_add_vertex(p0);
    uint32_t v1 = _shape_add_vertex(p1);
    uint32_t v2 = _shape_add_vertex(p2);
    uint32_t v3 = _shape_add_vertex(p3);
...
void Editor::_shape_preview_draw(ImDrawList* dl) const
{
    ...
    float S = m_shape.radius * px_per_world;                      // half-size in px
    ImU32 fc = ms_SHAPE_PREVIEW_FILL_COLOR;
    ImU32 lc = ms_SHAPE_PREVIEW_LINE_COLOR;

    if (m_shape.kind == ShapeKind::Rectangle) {
        dl->AddRectFilled({cen_px.x - S, cen_px.y - S},
                          {cen_px.x + S, cen_px.y + S},
                          fc, 0.0f);
        dl->AddRect({cen_px.x - S, cen_px.y - S},
                    {cen_px.x + S, cen_px.y + S},
                    lc, 0.0f, 0, ms_SHAPE_PREVIEW_LINE_WIDTH);
    } else if (m_shape.kind == ShapeKind::Ellipse) {
        dl->AddCircleFilled(cen_px, S, fc, 32);
        dl->AddCircle(cen_px, S, lc, 32, ms_SHAPE_PREVIEW_LINE_WIDTH);
    }
}
