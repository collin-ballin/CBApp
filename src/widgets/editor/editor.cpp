    ...
    if ( this->m_save_dialog.is_open() )
    {
        ...
        if ( this->m_save_dialog.Begin() )      // returns true when finished
        {
            ...
            if ( auto path = this->m_save_dialog.result() )
            {
                ...
                {
                    ES.m_filepath       = *path;
                    CB_LOG( LogType::Info, "Editor: saved data to file \"{}\"", ES.m_filepath.filename().string() );
                }
                //
                //  CASE 1.1B.      SAVE FAILURE...
                else
                {
                    CB_LOG( LogType::Error, "Editor: failure to save data to file \"{}\"", (*path).filename().string() );
                    //
            ...
            {
                CB_LOG( LogType::Debug, "Editor: cancelled file dialog menu (\"save file\")" );
            }
    ...
    if ( this->m_open_dialog.is_open() )
    {
        if ( this->m_open_dialog.Begin() )      // returns true when finished
        {
            ...
            if ( auto path = this->m_open_dialog.result() )
            {
                ...
                {
                    ES.m_filepath       = *path;
                    CB_LOG( LogType::Info, "Editor: loaded session from file \"{}\"", ES.m_filepath.filename().string() );
                }
                //
                //  CASE 2.1B.      LOAD FAILURE...
                else
                {
                    CB_LOG( LogType::Error, "Editor: failure to load file \"{}\"", (*path).filename().string() );
                    ES.m_filepath       = fs::path{   };
                    this->RESET_ALL();
                }
            ...
            {
                CB_LOG( LogType::Debug, "Editor: cancelled file dialog menu (\"open file\")" );
            }
