...
def mock_packets():
    ...
    while True:
        ...
        # Optional: add Poisson jitter to look more realistic
        jittered = [int(random.poisson(mu) if mu > 20 else mu) 
                    if hasattr(random, "poisson") else mu  # Python <3.12 fallback
                    for mu in counts]
    ...
    if mock_mode:
            ...
            try:
                while True:
                    ...
                    if key == "duration":
                        ...
                        win_value = val   # kept only for parity
                    elif key == "quit":
                        return
            except queue.Empty:
                pass
        ...
        with session:
            ...
            while True:
                ...
                start_measure(session, win_value)
                time.sleep(duration)
